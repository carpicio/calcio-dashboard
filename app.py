...
st.set_page_config(page_title="⚽ Dashboard V46 TOTAL", layout="wide", page_icon="⚽")
warnings.simplefilter(action='ignore', category=FutureWarning)

# --- TITOLO ---
st.title("💎 Dashboard V46: Analisi Totale")
st.markdown("**Statistiche • ELO • Money Management • Poisson • Ritmi**")
st.divider()
...
with st.sidebar:
    ...
    user_elo_h = ce1.number_input("ELO Casa (Manuale)", value=1500, step=10)
    user_elo_a = ce2.number_input("ELO Ospite (Manuale)", value=1500, step=10)
    ...
    q_1 = c_q1.number_input("Quota 1", value=1.00, step=0.01, format="%.2f")
    q_x = c_qx.number_input("Quota X", value=1.00, step=0.01, format="%.2f")
    q_2 = c_q2.number_input("Quota 2", value=1.00, step=0.01, format="%.2f")
...
if st.button("🚀 AVVIA ANALISI COMPLETA", type="primary"):
    ...
    avg_h_conc_2t = safe_div(goals_h['S_2T'], match_h)
    ...
    avg_a_conc_2t = safe_div(goals_a['S_2T'], match_a)
    ...
    with c1:
        ...
        st.write(f"**1°T:** F {avg_h_ht:.2f} | S {avg_h_conc_ht:.2f}")
        st.write(f"**2°T:** F {avg_h_2t:.2f} | S {avg_h_conc_2t:.2f}")
        st.write(f"**FIN:** F {avg_h_ft:.2f} | S {avg_h_conc_ft:.2f}")
    with c2:
        st.warning(f"**✈️ {sel_away}** ({match_a} match)")
        st.write(f"⏱️ Minuto medio 1° Gol: **{int(avg_min_a)}'**")
        st.write(f"**1°T:** F {avg_a_ht:.2f} | S {avg_a_conc_ht:.2f}")
        st.write(f"**2°T:** F {avg_a_2t:.2f} | S {avg_a_conc_2t:.2f}")
        st.write(f"**FIN:** F {avg_a_ft:.2f} | S {avg_a_conc_ft:.2f}")
    ...
    with col_mod1:
        ...
        st.write(f"X: **{px_ft*100:.1f}%** (@{to_odd(px_ft)})")
        st.write(f"2: **{p2_ft*100:.1f}%** (@{to_odd(p2_ft)})")
        st.caption(f"0-0 HT: {prob_00_ht*100:.1f}% | U1.5 HT: {prob_u15_ht*100:.1f}%")
    ...
    with col_mod3:
        st.markdown("#### 📜 ELO Storico")
        if has_elo and hist_elo_h:
            avg_hist_h = safe_mean(hist_elo_h)
            avg_hist_a = safe_mean(hist_elo_a)
            st.write(f"Casa: {int(avg_hist_h)}")
            st.write(f"Ospite: {int(avg_hist_a)}")
        else:
        ...
        if quota > 1.0:
            ...
            with cols[i % 3]:
                ...
                <div style="border:1px solid #555; padding:10px; border-radius:5px; margin-bottom:10px;">
                    ...
                    Fair: <b>{fair:.2f}</b> | Val: {valore*100:.1f}%<br>
    ...
    # --- GRAFICI ---
    tab1, tab2, tab3, tab4 = st.tabs(["📉 Ritmo Gol (KM)", "⚽ Heatmap Fatti", "🛡️ Heatmap Subiti", "🎯 Matrice Poisson"])
